...
static int	sphere_intersection(t_mlx *mlx, t_vec3 distance_vector, double projection)
{
	double	distance_sq;
	double	radius_sq;
	double	new_length;
...
int	sphere_intersection_init(t_mlx *mlx, t_vec3 origin, t_vec3 direction)
{
	...
	if ((projection = vector_dot_product(origin_to_sphere, direction)) < 0)
		return (0);
	distance_vector = vector_substraction(origin_to_sphere, vector_double_product(mlx->cam->ray[mlx->map->light_count]->direction, projection));
